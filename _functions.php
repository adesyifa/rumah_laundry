...
function add_kary($karyawan){
	...
	if(empty($_SESSION['master'])){$level= 'user';}
	else{$level	= $karyawan['level'];}
...
function update_kary($up_kary){
	...
	$password   = password_hash(htmlspecialchars($up_kary['password']), PASSWORD_DEFAULT);
...
function order_ck($order_ck){
	...
	$user = htmlspecialchars($_SESSION['master']);
...
function order_dc($order_dc){
	...
	$query_dc = "INSERT INTO tb_order_dc VALUES (
		...
		'$berat_dc','$trf_dc','$tgl_msk_dc','$tgl_kel_dc','$tot_bayar_dc','$ket_dc','$user'
	)";
...
function order_cs($order_cs){
	...
	$query_cs = "INSERT INTO tb_order_cs VALUES (
		...
		'$wkt_krj_cs','$jml_pcs','$trf_cs','$tgl_msk_cs','$tgl_kel_cs','$totBayar_cs','$ket_cs','$user'
	)";
	mysqli_query($koneksi,$query_cs);
	return mysqli_affected_rows($koneksi);
}
